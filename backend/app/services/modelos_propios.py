...
Los modelos viven en memoria del proceso y se pierden al reiniciar. En el
plan gratuito no hay disco persistente ni base de datos. Para produccion se
serializan a Object Storage con el identificador como nombre, igual que el
modelo principal.
"""
...
async def entrenar(archivo: UploadFile, nombre: str) -> dict:
    ...
    return {"id": identificador, **{k: v for k, v in _modelos[identificador].items() if k != "modelo"}}
...
def clasificar(identificador: str, texto: str, top_n: int = 3) -> dict:
    ...
    if guardado is None:
        raise HTTPException(
            ...
            "No existe ese modelo. Los modelos viven en memoria y se pierden "
            "al reiniciar el servicio; hay que volver a entrenarlo.",
        )
...
def listar() -> list:
    return [
        {"id": k, **{x: y for x, y in v.items() if x != "modelo"}}
